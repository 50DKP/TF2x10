...
public OnPluginStart()
{
	 ...
	 * ConVars *
	 ***********/
	new Handle:cv_version = CreateConVar("tf2items_rnd_version", PLUGIN_VERSION, "[TF2Items]Randomizer x10 Version", FCVAR_NOTIFY | FCVAR_PLUGIN | FCVAR_SPONLY);
...
#if defined _steamtools_included
	new Handle:cv_gamedesc = CreateConVar("tf2items_rnd_gamedesc", "1", "1 - [TF2Items]Randomizer x10 vVERSION when Randomizer is enabled. Requires Steamtools.", FCVAR_PLUGIN, true, 0.0, true, 1.0);
#endif
...
public cvhook_enabled(Handle:cvar, const String:oldVal[], const String:newVal[])
{
	...
	if (bCvarEnabled)
	{
		...
		PrintToChatAll("[TF2Items]Randomizer x10 Enabled!");
#if defined _steamtools_included
		if (steamtools && bCvarGameDesc)
		{
			decl String:gameDesc[64];
			Format(gameDesc, sizeof(gameDesc), "[TF2Items]Randomizer x10 v%s", PLUGIN_VERSION);
	...
	else
	{
		...
		PrintToChatAll("[TF2Items]Randomizer x10 Disabled!");
#if defined _steamtools_included
		if (steamtools && bCvarGameDesc)
		{
...
//	else
	if (bCvarEnabled) ReplyToCommand(client, "[TF2Items]Randomizer x10 is already enabled!");
...
{
	if (!bCvarEnabled) ReplyToCommand(client, "[TF2Items]Randomizer x10 is already disabled!");
	else if (bCvarEnabled)
	{
...
public Action:Timer_Announce(Handle:timer, any:userid)
{
	...
	if (bCvarEnabled && bCvarAnnounce && IsValidClient(client) && !bHasAnnounce[client])
	{
		...
		PrintToChat(client, "--\x0794DBFFTF2x10 \x01Plugin \x0794DBFFDevelopment \x01by \x0794DBFFIsatis\x01.");
...
/*		SetHudTextParams(-1.0, 0.1, 5.0, 255, 255, 255, 255,0,0.2,0.0,0.1);
		if (class == TFClass_Spy && iRndCloak[client] > -1) ShowHudText(client, -1, "[TF2Items]Randomizer x10:\n%s\n%s\n%s\n%s", strWeaponPrimary[pri], strWeaponSecondary[sec], strWeaponMelee[mel], strWeaponCloakary[iRndCloak[client]]);
		else ShowHudText(client, -1, "[TF2Items]Randomizer x10\n%s\n%s\n%s", strWeaponPrimary[pri], strWeaponSecondary[sec], strWeaponMelee[mel]); */
//		if (class == TFClass_Spy && iRndCloak[client] > -1) PrintHintText(client, "[TF2Items]Randomizer x10: %s, %s, %s, %s", strWeaponPrimary[pri], strWeaponSecondary[sec], strWeaponMelee[mel], strWeaponCloakary[iRndCloak[client]]);
//		else PrintHintText(client, "[TF2Items]Randomizer x10: %s, %s, %s", strWeaponPrimary[pri], strWeaponSecondary[sec], strWeaponMelee[mel]);
		// primary
		if (pri >= sizeof(iWeaponMelee))
		{
			LogError("[TF2Items]Randomizer x10 had primary weapon index %d on client %d %N, this is invalid", pri, client, client);
			pri = 0;
		}
		if (pri > 0)
		{
//			RemovePlayerBooties(client);
		...
		{
			LogError("[TF2Items]Randomizer x10 had secondary weapon index %d on client %d %N, this is invalid", sec, client, client);
			sec = 0;
		}
		if (sec > 0)
		{
...
//			RemovePlayerBack(client);
		...
		{
			LogError("[TF2Items]Randomizer x10 had melee weapon index %d on client %d %N, this is invalid", mel, client, client);
			mel = 0;
		}
		if (mel > 0)
		{
...
public Action:Timer_ShowInventory(Handle:timer, any:userid)
{
	...
	if (class == TFClass_Spy && iRndCloak[client] > -1) ShowSyncHudText(client, hWeaponsHud, "[TF2Items]Randomizer x10\n%s\n%s\n%s\n%s", strWeaponPrimary[pri], strWeaponSecondary[sec], strWeaponMelee[mel], strWeaponCloakary[iRndCloak[client]]);
	else ShowSyncHudText(client, hWeaponsHud, "[TF2Items]Randomizer x10\n%s\n%s\n%s", strWeaponPrimary[pri], strWeaponSecondary[sec], strWeaponMelee[mel]);
	return Plugin_Continue;
}
public Action:Command_MyLoadout(client, args)
{
	...
	new pri = iRndWeapon[client][0], sec = iRndWeapon[client][1], mel = iRndWeapon[client][2];
	if (pri < 0) pri = 0;
	if (class == TFClass_Spy && iRndCloak[client] > -1) Format(message, sizeof(message), "[TF2Items]Randomizer x10: %s, %s, %s, %s", strWeaponPrimary[pri], strWeaponSecondary[sec], strWeaponMelee[mel], strWeaponCloakary[iRndCloak[client]]);
	else Format(message, sizeof(message), "[TF2Items]Randomizer x10: %s, %s, %s", strWeaponPrimary[pri], strWeaponSecondary[sec], strWeaponMelee[mel]);
...
public Action:Command_CantSee(client, args)
{
	...
	if (GetEntityClassname(entity, classname, sizeof(classname)) && strncmp(classname, "tf_weapon_", 10, false) == 0)
	{
		...
		else
		{
			...
			ReplyToCommand(client, "[TF2Items]Randomizer x10: Made your active weapon transparent.");
		}
	}
	return Plugin_Handled;
}
	...
	for (new client = 1; client <= MaxClients; client++)
	{
		...
		if (IsClientInGame(client) && IsPlayerAlive(client))
		{
			...
			if (class != TFClass_Scout && GetIndexOfWeaponSlot(client, TFWeaponSlot_Primary) == 772)
			{
				...
				{
					TF2Attrib_SetByName(client, "major move speed bonus", 1.0 + flCurrHype / 100.0);
	...
	if (!tf2items_giveweapon)
	{
		if ((buttons & IN_ATTACK2) && !(iLastButtons[client] & IN_ATTACK2))
		{
			...
			if ((buttons & IN_RELOAD) && GetEntityMoveType(client) != MOVETYPE_NONE && ((targe = FindPlayerTarge(client)) != -1) && hTimers[client][ChargeTimer] == INVALID_HANDLE && GetEntPropFloat(client, Prop_Send, "m_flMaxspeed") > 1.01 && (class != TFClass_DemoMan || (wep == GetPlayerWeaponSlot(client, TFWeaponSlot_Melee) && (idxslot2 == 265 || idxslot2 == 20 || idxslot2 == 207 || idxslot2 == 130))))
			{
				...
				if (GetIndexOfWeaponSlot(client, TFWeaponSlot_Melee) == 327) chargetime = 6.5;
				else chargetime = 1.5;
...
public Action:Timer_TargeCharging(Handle:timer, any:userid)
{
	...
	if (GetIndexOfWeaponSlot(client, TFWeaponSlot_Melee) == 327) charge -= (0.1 / 6.5 * 100.0);
...
//			if (idx == 433) SetDalokohs(client, true);
		...
		if (StrEqual(weapon3, "tf_weapon_lunchbox_drink") && TF2_GetPlayerClass(client) != TFClass_Scout)
		{
			...
			hTimers[client][BonkTimer] = CreateTimer(11.4, Bonk_Cooldown, GetClientUserId(client));
			new bool:megaheal = TF2_IsPlayerInCondition(client, TFCond_MegaHeal);
			TF2_RemoveCondition(client, TFCond_MegaHeal);
			...
			SetEntPropFloat(sec, Prop_Send, "m_flEffectBarRegenTime", GetGameTime() + 11.4);
		}
	}
	return buttons;
}
...
SetSandvich(client)
{
	...
	if (GetClientHealth(client) < TF2_GetMaxHealth(client))
	{
		...
		{
			SetSpeshulAmmo(client, TFWeaponSlot_Secondary, GetSpeshulAmmo(client, TFWeaponSlot_Secondary)-1);
			new sec = GetPlayerWeaponSlot(client, TFWeaponSlot_Secondary);
...
public Action:Eat_CooldownTime(Handle:timer, any:userid)
{
	...
	if (bCooldowns[client][EatCooldown])
	{
		...
		if (bCooldowns[client][LongEatCooldown])
		{
			if (IsValidClient(client))
			{
				PrintHintText(client, "[TF2Items]Randomizer x10: Your Food has Recharged");
				EmitSoundToClient(client, "player/recharged.wav");
			}
...
public Action:Bonk_Cooldown(Handle:timer, any:userid)
{
	...
	if (bCooldowns[client][BonkCooldown])
	{
		...
		if (IsValidClient(client))
		{
			PrintHintText(client, "[TF2Items]Randomizer x10: Your Drink has Recharged");
			EmitSoundToClient(client, "player/recharged.wav");
		}
	}
	if (GetSpeshulAmmo(client, TFWeaponSlot_Secondary) < 1) SetSpeshulAmmo(client, TFWeaponSlot_Secondary, 1);
...
public Action:Ball_Cooldown(Handle:timer, any:userid)
{
	...
	if (bCooldowns[client][BallCooldown])
	{
		...
		if (IsValidClient(client))
		{
			PrintHintText(client, "[TF2Items]Randomizer x10: Your Ball has Recharged");
			EmitSoundToClient(client, "player/recharged.wav");
		}
	}
...
public Action:Jar_Cooldown(Handle:timer, any:userid)
{
	...
	if (bCooldowns[client][JarCooldown])
	{
		...
		if (valid)
		{
			PrintHintText(client, "[TF2Items]Randomizer x10: Your Jar has Recharged");
			EmitSoundToClient(client, "player/recharged.wav");
		}
	}
...
//sandvich
	...
	SetTrieValue(hItemInfoTrie, "42_ammo", 10, false);
...
stock GiveWeaponOfIndex(client, weaponLookupIndex)
{
	...
	if (!isValidItem)
	{
		PrintToChat(client, "[TF2Items]Randomizer x10: Error! Tried to give you nonexistent weapon %d.", weaponLookupIndex);
		return -1;
	}
...
/*public Action:OnGetGameDescription(String:gameDesc[64])
{
	if (bCvarEnabled && bCvarGameDesc && (g_bMapLoaded || !cvar_manifix))
	{
		...
		Format(g_szGameDesc, 64, "%s v%s", "[TF2Items]Randomizer x10", PLUGIN_VERSION);
